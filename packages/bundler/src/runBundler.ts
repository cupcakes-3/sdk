...
import { erc4337RuntimeVersion } from '@cupcakes-sdk/common'
...
export async function runBundler(argv: string[], overrideExit = true): Promise<BundlerServer> {
  ...
  program
    ...
    .option('--port <number>', 'server listening port', '9080')
  ...
  const provider: BaseProvider =
    ...
    config.network === 'hardhat' ? require('hardhat').ethers.provider : ethers.getDefaultProvider(config.network)
  let mnemonic: string
  let wallet: Wallet
  try {
    // mnemonic = fs.readFileSync(config.mnemonic, 'ascii');
    // wallet = Wallet.fromMnemonic(mnemonic).connect(provider);
    wallet = new ethers.Wallet(config.mnemonic, provider)
  } catch (e: any) {
  ...
  const methodHandler = new UserOpMethodHandler(provider, wallet, config, entryPoint, bundlerHelper)

  const bundlerServer = new BundlerServer(methodHandler, config, provider, wallet)
