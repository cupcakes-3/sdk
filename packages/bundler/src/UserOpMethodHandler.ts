...
export class UserOpMethodHandler {
  ...
  async sendUserOperation(
    ...
    try {
      ...
      ({ estimated, factored } = await this.estimateGasForHelperCall(
        userOp,
        beneficiary
      ));
    } catch (error: any) {
  ...
  async estimateGasForHelperCall(
    ...
    const estimateGasRet = await this.bundlerHelper.estimateGas.handleOps(
      0,
      this.config.entryPoint,
      [userOp],
      beneficiary
    ...
    const factored = estimated
      .mul(Math.round(parseFloat(this.config.gasFactor) * 100000))
      .div(100000);
    return { estimated, factored };
  }

  async printGasEstimationDebugInfo(
    userOp1: UserOperationStruct,
    beneficiary: string
  ): Promise<void> {
    const userOp = await resolveProperties(userOp1);
    ...
    const [estimateGasRet, estHandleOp, staticRet] = await Promise.all([
      this.bundlerHelper.estimateGas.handleOps(
        0,
        this.config.entryPoint,
        [userOp],
        beneficiary
      ...
      this.bundlerHelper.callStatic.handleOps(
        0,
        this.config.entryPoint,
        [userOp],
        beneficiary
    ...
    const estimateGasFactored = estimateGas
      .mul(Math.round(parseInt(this.config.gasFactor) * 100000))
      .div(100000);
    ...
    console.log(
      'preVerificationGas',
      parseInt(userOp.preVerificationGas.toString())
    );
    // eslint-disable-next-line @typescript-eslint/no-base-to-string
    console.log(
      'verificationGas',
      parseInt(userOp.verificationGasLimit.toString())
    );
    // eslint-disable-next-line @typescript-eslint/no-base-to-string
    console.log('callGas', parseInt(userOp.callGasLimit.toString()));
    console.log(
      'Total estimated gas for bundler compensation: ',
      estimateGasFactored
    );
  }
}
